...
namespace MlxPep.Cli
{
    ...
    public static class CliBuilder
    {
        ...
        private static async Task<int> HandleApply(string[] args, bool isJson)
        {
            if (args.Length == 0)
            {
                return PrintErrorAndReturn1("Usage: mlx-pep apply <profile_file> [--dry-run] [--harness <name>]");
            }
            ...
            var context = new CommandContext(isJson);
            var result = await handler.ExecuteAsync(profile, harness, output, dryRun, backup, context);
