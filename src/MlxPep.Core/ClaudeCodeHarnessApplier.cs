...
/// <summary>
/// Applies a profile to Claude Code harness configuration.
/// Issue #24: OpenCode + Claude Code emitters
/// </summary>
public class ClaudeCodeHarnessApplier : IHarnessApplier
{
    public string HarnessName => "claude-code";
        ...
        bool requestedInsiders = false)
    {
        try
        {
            // Validate that profile has claude-code harness block
            if (profile.Harness == null || !profile.Harness.ContainsKey("claude-code"))
            {
                return new HarnessApplyResult(
                    ...
                    IsDryRun: isDryRun,
                    Success: false,
                    Error: "Profile does not contain 'claude-code' harness configuration",
                    Changes: new List<FileChangeResult>());
            }

            // Determine config directory and file path
            var configDir = GetClaudeCodeConfigDirectory();
            var configFile = Path.Combine(configDir, "config.json");

            // Emit the config using ClaudeCodeEmitter
            ...
            if (File.Exists(configFile))
            {
                ...
                var (success, location, _) = await backupManager.CreateBackupAsync("claude-code", profile.Id, changes);
                if (success)
    ...
    private string GetClaudeCodeConfigDirectory()
    {
        var homeDir = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
        return Path.Combine(homeDir, ".claude-code");
    }
