...
done < <(read_profile_ids "$PROFILES_JSON")

if [[ ${#PROFILE_IDS[@]} -eq 0 ]]; then
  echo "No profiles found in $PROFILES_JSON" >&2
  exit 1
fi

for profile_id in "${PROFILE_IDS[@]}"; do
  echo "Phase 1/3: benchmark/probe collection for profile: $profile_id"
  ...
  echo "Phase 2/3: prompt-quality evaluation for profile: $profile_id"
...
echo "Phase 3/3: generate recommendation report after all benchmark and evaluation evidence is complete"
